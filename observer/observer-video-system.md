...
14. [⚙️ Оптимизация производительности](#14--оптимизация-производительности)
    - [14.1. Разрешение, FPS, Coral TPU](#141-выбор-разрешения-для-детекции)
    - [14.2. Аппаратное ускорение AMD GPU (VA-API + OpenVINO)](#142-аппаратное-ускорение-amd-gpu-va-api--openvino)
...
### ⚠️ Проблема toggle-кнопки Power

На большинстве бюджетных телевизоров (в том числе Samsung F5000) кнопка Power на пульте — это **toggle**: одно нажатие включает, следующее выключает. Отдельных ИК-кодов "только включить" и "только выключить" нет.

Это значит: если автоматизация сработает дважды пока ТВ уже включён — он выключится.

**Решение 1 — Cooldown (простое)**

Добавить условие: не отправлять команду чаще чем раз в N минут. Если ТВ включился — повторная команда не уйдёт пока не истечёт таймер.

```yaml
automation:
  - alias: "TV включить при обнаружении человека"
    trigger:
      - platform: state
        entity_id: binary_sensor.frigate_person  # сенсор из Frigate
        to: "on"
    condition:
      - condition: template
        value_template: >
          {{ (now() - state_attr('automation.tv_включить_при_обнаружении_человека', 'last_triggered') | default(now() - timedelta(hours=1))).total_seconds() > 600 }}
    action:
      - action: remote.send_command
        target:
          entity_id: remote.broadlink_rm4c_mini
...
`600` секунд = 10 минут. Подбери под свой сценарий.

**Решение 2 — Умная розетка с мониторингом потребления (надёжное)**

Умная розетка измеряет потребление тока. ТВ в режиме ожидания потребляет ~0.5–2 Вт, во включённом состоянии — 40–80 Вт. HA видит это и знает реальное состояние ТВ — команда отправляется только когда он выключен.

Подходящие розетки с поддержкой HA:

> 💡 Если есть Zigbee-координатор (донгл) — предпочитай Zigbee-розетки. Они работают локально без облака, не нагружают Wi-Fi и образуют mesh-сеть.

**Zigbee (рекомендуется):**

| Модель | Мониторинг мощности | Интеграция в HA | Примечание |
|---|---|---|---|
| **NOUS A1Z** | ✅ | Zigbee2MQTT | Лучший выбор: дёшево (~8–10€), хорошая поддержка |
| **Tuya TS011F** (Zigbee) | ✅ | Zigbee2MQTT | Очень распространена, много брендов на AliExpress |
| **Aqara SP-EUC01** | ✅ | Zigbee2MQTT | Надёжная, чуть дороже (~12–18€) |
| **Sonoff S26R2ZB** | ❌ | Zigbee2MQTT | Без мониторинга мощности — не подходит для этой задачи |
| **IKEA TRÅDFRI E1603** | ❌ | Zigbee2MQTT | Без мониторинга мощности — не подходит для этой задачи |

**Wi-Fi (если Zigbee недоступен):**

| Модель | Мониторинг мощности | Интеграция в HA |
|---|---|---|
| **Sonoff POW R3** | ✅ | SonoffLAN (локально) |
| **Tuya / BlitzWolf BW-SHP6** | ✅ | LocalTuya |

Для мониторинга мощности нужна розетка с **energy monitoring** — не все модели это умеют, проверяй характеристики перед покупкой.

Автоматизация с розеткой:
...
automation:
  - alias: "TV включить при обнаружении человека"
    trigger:
      - platform: state
        entity_id: binary_sensor.frigate_person
        to: "on"
    condition:
      - condition: numeric_state
        entity_id: sensor.smart_plug_power  # сенсор мощности розетки
        below: 5  # ТВ выключен если потребление < 5 Вт
    action:
      - action: remote.send_command
        target:
          entity_id: remote.broadlink_rm4c_mini
...
## 14.1. Выбор разрешения для детекции
...
## 14.2. Аппаратное ускорение AMD GPU (VA-API + OpenVINO)
